...
function wrapPage(id) {
  return wrap('<!doctype html><html><head>'
    ...
    + '<link rel=stylesheet href="http://evbogue.com/style.css">'
    + '</head><body>',
    '</body></html>'
  )
}
...
function render(opts, c)
{
	...
	return ' pushed to <a href="http://gitmx.com/'+ encodeURIComponent(c.repo) + '/">' + c.repo.substring(0, 10)+'...' + '</a><ul>' +  
          (c.commits != undefined ? 
           c.commits.map(com => { return '<li><code>' + com.sha1.substring(0, 10) + '</code> "' +  com.title + '"</li>' }).join('') : "</ul>")
...
function renderPost(opts, c) {
  if (c.root)
    return '<div class="ssb-post"> Re: <a href="/' + encodeURIComponent(c.root) + '">' +  c.root.substring(0, 10) + '</a><br />' + marked(c.text, opts.marked) + '</div>'
  else return '<div class="ssb-post">' + marked(c.text, opts.marked) + '</div>'
}
